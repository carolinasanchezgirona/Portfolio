(() => {
  ...
  const els = {
    ...
    newDocuments: $("#new-patient-documents"), newSignature: $("#new-patient-signature"), newInformation: $("#new-patient-information")
  };
  ...
  function updatePatientType() {
    const existing = form.elements.patient_type.value === "existing";
    els.newDocuments.hidden = existing;
    els.newSignature.hidden = existing;
    els.newInformation.hidden = existing;
    form.elements.privacy_acknowledged.required = !existing;
    form.elements.informed_consent_accepted.required = !existing;
    form.elements.signer_name.required = !existing;
    if (existing) {
      form.elements.privacy_acknowledged.checked = false;
      form.elements.informed_consent_accepted.checked = false;
      form.elements.signer_name.value = "";
    }
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!data.get("cancellation_accepted")) return showMessage("Debes leer la política de cancelación y asistencia.", "error");
    if (!isExisting && (!data.get("privacy_acknowledged") || !data.get("informed_consent_accepted"))) return showMessage("Debes leer y aceptar la privacidad y el consentimiento informado.", "error");
    if (!isExisting && !signer) return showMessage("Escribe tu nombre y apellidos en el campo de firma electrónica.", "error");
    if (!isExisting && signer.toLocaleLowerCase("es") !== name.toLocaleLowerCase("es")) return showMessage("La firma debe coincidir con el nombre y apellidos indicados.", "error");
    ...
    try {
      const response = await fetch(`${REST_URL}/rpc/create_calendar_booking`, {
        ...
        body: JSON.stringify({ p_starts_at: state.selected.starts_at, p_duration_minutes: state.duration, p_patient_name: name,
          p_patient_email: email, p_patient_phone: phone, p_patient_type: patientType,
          p_privacy_acknowledged: !isExisting, p_cancellation_accepted: true,
          p_informed_consent_accepted: !isExisting, p_signer_name: isExisting ? null : signer })
      });
      if (!response.ok) {
      ...
      showMessage(isExisting ? "Solicitud registrada. El horario queda reservado mientras comprobamos que eres paciente actual." : "Reserva registrada correctamente. Recibirás la confirmación por el canal indicado.", "success");
      updatePatientType();
      await loadSlots();
    } catch (error) {
